...
In step 3, when you run the ./insstall.sh, don't add the esp32 parameter.  That will only install esp32, not esp32c6.  Don't add a param for it to install all

> ./install.sh

You should finish with the source ./export.sh which adds environment variables needed by the SDK.
...
3. That you are using the esp32c6 mentioned above
4. You are plugging your USB cable into the MCU port that says "UART" not "USB"
5. That you have both MCUs plugged in.

Some of the referenced guides use a different MCU, such as esp32h2.  Don't be distracted by that, we are using esp32c6.

When flashing with both devices connected, you will need to specify the serial port.  Plug in the first and look in /dev to see the serial port for MCU 1, write that down, then plug in MCU 2 and write down the port for MCU 2.

On WSL2 using USB/IP, these are:  (If you are using WSL2 and have trouble with USB/IP, contact me)
...
We can skip the menuconfig step unless you need to change defaults, or use JTAG.
...
The response should be "leader".  Now we need to save some information that we will give to MCU2.  We will get the IPc6 addresses and the dataset in hex to cut and past into MCU2 later.
...
In the terminal window for MCU1

### Tcpsockclient on MCU2

We will open the tcpsockclient and connect it to MCU1 on port 12345.  Replace ADDRESS with one of the IPv6 addresses on MCU1. I had to try more than one address before it worked, you may need to as well.
The address that worked for me is the one suffixed by "16"
...
> tcpsockserver send Hello\ World
